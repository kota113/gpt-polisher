...
app.get("/", (c) => {
  ...
  return c.html(page("Gemini Rewrite MCP", `<main class="shell"><nav class="nav"><a class="brand" href="/">${logo}<span>Gemini Rewrite</span></a></nav><section class="hero"><span class="eyebrow"><span class="spark">✦</span> Powered by Gemini</span><h1>Make every response<br>clearer.</h1><p>An OAuth-protected MCP server that uses Gemini to refine AI responses while preserving their original meaning.</p><div class="endpoint"><code>${endpoint}</code></div></section><section class="guide"><header class="guide-head"><h2>Connect it to ChatGPT</h2><p>Connect the MCP server once, then add an instruction to make Gemini Rewrite the final editing step.</p></header><div class="guide-card"><div class="guide-section"><h3>1. Turn on Developer mode</h3><div class="steps"><div class="step"><span class="step-num">1</span><div class="step-body"><h4>Open ChatGPT settings</h4><p>In <a href="https://chatgpt.com/" target="_blank" rel="noreferrer">ChatGPT</a>, open <span class="inline-code">Settings → Security and login</span> and turn on <span class="inline-code">Developer mode</span>.</p></div></div></div></div><div class="guide-section"><h3>2. Connect Gemini Rewrite</h3><div class="steps"><div class="step"><span class="step-num">2</span><div class="step-body"><h4>Add the server URL</h4><p>Go to <a href="https://chatgpt.com/plugins" target="_blank" rel="noreferrer">ChatGPT Plugins</a>, select the plus button, and connect this server URL in Developer mode.</p><p><span class="inline-code">${endpoint}</span></p><p>Complete the Google OAuth and project-selection screens when prompted.</p></div></div></div></div><div class="guide-section"><h3>3. Add the Custom Instruction</h3><p>Paste this into ChatGPT’s Custom Instructions.</p><div class="instruction" id="rewrite-instruction">After drafting a response, always call the Gemini Rewrite <code>rewrite_response</code> tool and provide it with the original response and the user's question. Use only the rewritten text returned by the tool as your final answer.<button class="copy" type="button" onclick="navigator.clipboard.writeText(document.getElementById('rewrite-instruction').childNodes[0].textContent.trim());this.textContent='Copied'">Copy</button></div></div></div></section></main>`));
});
...
app.post("/setup/create", async (c) => {
  ...
  const state = String(body.state ?? "");
  const projectId = String(body.projectId ?? "");
  const displayName = String(body.displayName ?? "Gemini Rewrite MCP");
...
async function finishSetup(c: any, state: string, projectId: string, create: boolean, displayName?: string) {
  ...
  try {
    ...
    if (create) await createProject(accessToken, projectId, displayName ?? "Gemini Rewrite MCP");
